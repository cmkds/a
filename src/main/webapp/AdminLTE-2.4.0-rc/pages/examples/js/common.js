$(function() {
	...
	$("#gnb>li>a").on("mouseover focus", function() {
		if (beforEl)
			beforEl.children("img")
				.attr("src", beforEl.children("img").attr("src").replace("over.gif", "out.gif"));
	...
	$("#gnb").on("mouseleave", function() {
		$("#gnb ul:visible").slideUp("fast")
		if (beforeEl)
			beforEl.children("img").attr("src", beforeEl.children("img").attr("src").replace("over.gif"));
	})
	...
	$("#total_btn a").on("click", function() {
		...
		// "images/allmenu_btn_out.gif" --> "images/allmenu_btn_over.gif"
		return false;
	...
	$("#total_close a").on("click", function() {
		...
		$("#total_btn a img").attr("total_btn a img").attr("src").replace("over.gif", "out.gif");
		return false;
	});
	...
	/*날짜 표기*/
	var t = new Date();
	var y = t.getFullYear(); //2022
	var m = t.getMonth(); //5-1=4
	var d = t.getDate(); //24
